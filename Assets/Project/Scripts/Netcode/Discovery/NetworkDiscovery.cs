    ...
    where TResponse : INetworkSerializable, new()
{
    ...
    async Task ReceiveBroadcastAsync()
    {
        ...
        var segment = new ArraySegment<byte>(udpReceiveResult.Buffer, 0, udpReceiveResult.Buffer.Length);
        using var reader = new FastBufferReader(segment, Allocator.TempJob); // TODO: Change Allocator.Temp to Allocator.TempJob
        ...
        try
        {
            ...
            {
                using var writer = new FastBufferWriter(1024, Allocator.TempJob, 1024 * 64); // TODO: Change Allocator.Temp to Allocator.TempJob
